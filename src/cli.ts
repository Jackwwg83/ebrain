...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'ebrain' && (!args[0] || hasHelpFlag(args))) {
    const { ebrainAuthUsage } = await import('./commands/ebrain-auth.ts');
  ...
  try {
    switch (command) {
      ...
      case 'ebrain': {
        const { runEbrainAuth } = await import('./commands/ebrain-auth.ts');
        process.exit(await runEbrainAuth(engine, args));
      }
      case 'agent': {
...
ADMIN
  ...
  ebrain register-client <name>      Register an Ebrain OAuth client
