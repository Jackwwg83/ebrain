  ...
  while (i < args.length) {
    ...
    switch (flag) {
      ...
      case '--token-endpoint-auth-method':
        ...
        i += 2; break;
      case '--executive-id': {
        const v = requireValue().trim();
        if (!v) throw new Error('--executive-id <id> cannot be empty');
        out.executiveId = v;
        i += 2; break;
      }
      default:
...
async function registerClient(name: string, args: string[]) {
  if (!name) {
    console.error('Usage: auth register-client <name> [--executive-id ID] [--grant-types G] [--scopes S] [--source SOURCE] [--federated-read SRC1,SRC2,...] [--redirect-uri URI ...] [--token-endpoint-auth-method client_secret_post|client_secret_basic|none]');
    process.exit(1);
  }
  ...
  const { grantTypes, scopes, sourceId, federatedRead, redirectUris, tokenEndpointAuthMethod, executiveId } = parsed;
  ...
  try {
    await withConfiguredSql(async (sql) => {
      ...
      const { clientId, clientSecret } = await provider.registerClientManual(
        name, grantTypes, scopes, redirectUris, sourceId, federatedRead, tokenEndpointAuthMethod, executiveId,
      );
