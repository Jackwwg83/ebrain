...
<div class="prompt">&ldquo;Give me the GBrain admin login link&rdquo;</div>
  ...
  app.post('/admin/api/register-client', requireAdmin, express.json(), async (req: Request, res: Response) => {
    try {
      ...
      const executiveId = typeof req.body?.executiveId === 'string'
        ? req.body.executiveId.trim()
        : typeof req.body?.executive_id === 'string'
          ? req.body.executive_id.trim()
          : undefined;
      const result = await oauthProvider.registerClientManual(
        name, grants, scopeString, uris, 'default', undefined, validatedAuthMethod, executiveId || undefined,
      );
