...
version: 0.1.0
description: Capital allocation tradeoff sketch for CFO and CEO operating decisions.
triggers:
  - "capital allocation advisor"
  - "capital allocation tradeoff"
  - "cfo allocation sketch"
  - "budget tradeoff memo"
  - "资本配置建议"
mutating: true
writes_pages: true
writes_to:
...
# capital-allocation-advisor — Capital Allocation Tradeoff Sketch
...
Frame allocation tradeoffs among growth, runway, headcount, product, and risk using evidence-backed constraints. The later full skill should support decisions, not automate them.
...
Use this skill for: budget tradeoff memos, CFO allocation sketches, runway versus growth decisions, headcount allocation, and investment option comparison.
...
Markdown with `## Decision`, `## Options`, `## Tradeoffs`, `## Constraints`, `## Recommendation`, and `## Evidence`.

## Filing

- Write durable synthesis under `analysis/capital-allocation/`.
- Include frontmatter: `generated_by: capital-allocation-advisor`, `period`, `executive_id`,
...
phrase `capital allocation advisor`. Fixtures include context around the trigger so they
...
- Do not write into a different namespace than `analysis/capital-allocation/`.
